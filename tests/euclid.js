...
QUnit.module('Тестируем функцию euclid', function () {
	...
	QUnit.test('Функция должна правильно обрабатывать некорректные аргументы', function (assert) {
		...
		assert.throws(function () {
				euclid(1, 2, 3.3)
			}, TypeError('Bad input parameters.'),
			'euclid(1, 2, 3.3) throws TypeError(\'Bad input parameters.\')');

		assert.throws(function () {
				euclid(1, 'a', 1)
			}, TypeError('Bad input parameters.'),
			'euclid(1, \'a\', 1) throws TypeError(\'Bad input parameters.\')');

		assert.throws(function () {
				euclid(-100)
			}, TypeError('Bad input parameters.'),
			'euclid(-100) throws TypeError(\'Bad input parameters.\')');
	});
});
